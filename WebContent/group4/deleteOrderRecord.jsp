...
<%
   ...
   // if(laboratoryID==null)
    ...
    if(oid!=null){
        int oidvalue=Integer.parseInt(oid);
        int k=-1;
        try {
            k=DAOFactory.getIResDAOInstance().deleteOrder(oidvalue,weekvalue);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if(k==1){
            out.print("<script>alert('删除成功!'); window.location.reload();</script>");
        }

    }
%>
<div class="table">
    <form action="deleteOrderRecord.jsp">
        <select name="week">
            <%
                for (int i=1;i<=20;i++){
                ...
                for(int j=0;j<7;j++){
                    if(table[i][j]==null)
                        ...
                        out.println("<td class='content'>"+table[i][j]
                            +"<a href='deleteOrderRecord.jsp?oid="+orderID[i][j]+"&week="+week+"' >删除</a></td>");
                    }
                }out.println("</tr>");
            }
